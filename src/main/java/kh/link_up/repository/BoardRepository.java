...
public interface BoardRepository extends JpaRepository<Board, Long> {
        ...
        @Query(value = "SELECT * FROM board WHERE category <> :excludeCategory AND MATCH(b_title) AGAINST(:text IN BOOLEAN MODE)", countQuery = "SELECT COUNT(*) FROM board WHERE category <> :excludeCategory AND MATCH(b_title) AGAINST(:text IN BOOLEAN MODE)", nativeQuery = true)
        Page<Board> searchByTitleForUsers(@Param("text") String text,
                        @Param("excludeCategory") String excludeCategory,
                        Pageable pageable);

        @Query(value = "SELECT * FROM board WHERE category <> :excludeCategory AND MATCH(b_content) AGAINST(:text IN BOOLEAN MODE)", countQuery = "SELECT COUNT(*) FROM board WHERE category <> :excludeCategory AND MATCH(b_content) AGAINST(:text IN BOOLEAN MODE)", nativeQuery = true)
        Page<Board> searchByContentForUsers(@Param("text") String text,
                        @Param("excludeCategory") String excludeCategory,
                        Pageable pageable);

        @Query(value = "SELECT * FROM board WHERE category <> :excludeCategory AND b_writer LIKE CONCAT(:text, '%')", countQuery = "SELECT COUNT(*) FROM board WHERE category <> :excludeCategory AND b_writer LIKE CONCAT(:text, '%')", nativeQuery = true)
        ...
        // @Query("SELECT b FROM Board b WHERE REPLACE(b.title, ' ', '') LIKE
        // REPLACE(CONCAT('%', :title, '%'), ' ', '')")
        // Page<Board> searchByTitle(@Param("title") String title, Pageable pageable);
        // 밑에 꺼가 인덱스 적용 한거
        @Query(value = "SELECT * FROM Board b WHERE MATCH(b_title) AGAINST(:title IN BOOLEAN  MODE)", countQuery = "SELECT COUNT(*) FROM Board b WHERE MATCH(b_title) AGAINST(:title IN BOOLEAN  MODE)", nativeQuery = true)
        Page<Board> searchByTitle(@Param("title") String title, Pageable pageable);
        ...
        // Page<Board> searchByWriter(@Param("writer") String writer, Pageable
        // pageable);
        @Query(value = "SELECT * FROM board b WHERE b_writer LIKE CONCAT(:writer, '%')", countQuery = "SELECT COUNT(*) FROM board b WHERE b_writer LIKE CONCAT(:writer, '%')", nativeQuery = true)
        ...
        // Page<Board> searchByContent(@Param("content") String content, Pageable
        // pageable);
