...
//    Page<Board> searchByTitle(@Param("title") String title, Pageable pageable);
    @Query(value = "SELECT * FROM Board b WHERE MATCH(b_title) AGAINST(:title IN NATURAL LANGUAGE MODE)",
            countQuery = "SELECT COUNT(*) FROM Board b WHERE MATCH(b_title) AGAINST(:title IN NATURAL LANGUAGE MODE)",
            nativeQuery = true)
    Page<Board> searchByTitle(@Param("title") String title, Pageable pageable);
